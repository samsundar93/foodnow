<?php if($action == 'categorystatuschange' && $field == 'status') { ?>
    <?php if($status == 'active'){?>
        <div class="label label-table label-success">
            <a class="statusColor" onclick="changeStatus('<?= $id ?>', '0', '<?= $field ?>', 'category/ajaxaction', 'categorystatuschange')">
                <i class="fa fa-check"></i>
            </a>
        </div>
    <?php }else {?>
        <div class="label label-table label-danger">
            <a class="statusColor" type="button" onclick="changeStatus('<?= $id ?>', '1', '<?= $field ?>', 'category/ajaxaction', 'categorystatuschange')">
